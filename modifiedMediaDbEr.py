...
db = client['media-db']
es_index='index-02aug19-r'
es_mapping='mapping-02aug19-r'
#--------------------------------
...
curl -X PUT '10.237.26.159:9200/index-02aug19-r?pretty' -H 'Content-Type: application/json' -d'
{
    "settings" : {
        "index" : {
            "number_of_shards" : 5,
...
curl -X PUT '10.237.26.159:9200/index-02aug19-r/_mapping/mapping-02aug19-r?pretty' -H 'Content-Type: application/json' -d '{"properties": {}}'
'''
...
def level0():
    ...
    coll_name ='entitiesForComparison1'# to change unresolved collection name --Rohit
    ...
    for chunk in range(0,1):
        ...
        mongo_coll='resolvedEntitySet_R'# to change resolved collection name --Rohit
...
def levelPlus1():
    ...
    for chunk in range(1,16,2**level):
        ...
        resolve(unresolved_coll, ent_to_hightfidf, resolved_col, cursor, chunk, chunk_size)
        cursor = unresolved_col2.find({}, no_cursor_timeout=True)
        resolve(unresolved_col2, ent_to_hightfidf, resolved_col, cursor, chunk, chunk_size)
...
def liveER():
    ...
    resolve(unresolved_coll, ent_to_hightfidf, resolved_col, cursor, chunk_size)
